...
| **IR** | Instruction Register (32-bit, with field breakdowns) |
| **A / B** | Temporary registers feeding the ALU |
| **ALU** | Arithmetic Logic Unit (6 operations) |
| **Registers** | 16 × 32-bit general-purpose register file |
| **Memory** | 2¹⁶ × 32-bit addressable memory |
| **Sign Extend** | Offset sign extension unit |
| **Comparison Logic** | Drives CmpTrue signal to the microcontroller |
...
| Input | Action |
|---|---|
| `→` / Next button | Advance one microcode cycle |
| `←` / Prev button | Go back one cycle |
| Show Fetch toggle | Include/exclude the FETCH0 cycle |
| Animate Flow toggle | Animate data movement on the bus |
